...
const LEGACY_FORMAT_REGEX = /((?:\w*-){1,}\w*)-(\d+)\.json$/;
export const migrateLegacyFormat = async (directory) => {
  ...
  for (let i = 0; i < files.length; i += 1) {
    ...
    if (match) {
      ...
      const newFile = `${md5(requestKey)}-${callIndex}.json`;

      // eslint-disable-next-line no-console
      console.log(`Renaming ${file} \t ${newFile}`);

      // eslint-disable-next-line no-await-in-loop
      await fse.rename(`${directory}/${file}`, `${directory}/${newFile}`);
    }
  }
};
